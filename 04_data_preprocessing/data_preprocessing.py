...
class DATA_PREPROCESSING:
    ...
    # def remove_outliers_inplace(self, percentile=97):
    ...
    #             print(f"⚠️ Cluster {cluster_id} übersprungen: zu wenige Datenpunkte ({X.shape[0]} < {X.shape[1]})")
    #             continue
...
if __name__ == "__main__":
    data_flash = DATA_PREPROCESSING(dir_name='data')   # Initialisiert die Datenpipeline
    ...
    data_flash.remove_outliers_inplace(which="inputs", percentile=97)
    data_flash.remove_outliers_inplace(which="outputs", percentile=97)
